...
public class ImageWriter {
    ...
    private static double[] flipY(double[] d, int w, int h) {
        double[] r = new double[w * h * 3];
        for (int y = 0; y != h; y++) {
            for (int x = 0; x != w; x++) {
                var id = ((h - y - 1) * w + x) * 3;
                var ir = (y * w + x) * 3;
    ...
    // created.
    public static boolean writeHdr(String basename, double[] dataIn, int width,
            int height) {

        double[] data = flipY(dataIn, width, height);
    ...
    public static void writePng(String basename, double[] dataIn, int width,
            int height) {
